...
public class TrigTest extends TestHardwareMap{
    ...
    public void loop() {
        ...
        Orientation orientation = navxGyro.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.XYZ, AngleUnit.DEGREES);
        ...
        if (leftstick_x > 0 && leftstick_y < 0) {//quadrant up/right
            myangle = (float) (90 + Math.toDegrees(Math.atan((Math.cos(gyroDegrees) * leftstick_y) / (Math.sin(gyroDegrees) * leftstick_x)))); //90 to 0
        }
        else if (leftstick_x > 0 && leftstick_y > 0) {//quadrant down/right
            myangle = (float) (90 + Math.toDegrees(Math.atan((Math.cos(gyroDegrees) * leftstick_y) / (Math.sin(gyroDegrees) * leftstick_x)))); //180 to 90}
        }
        else if(leftstick_x < 0 && leftstick_y > 0) {//quadrant down/left
            myangle = (float) (270 + Math.toDegrees(Math.atan((Math.cos(gyroDegrees) * leftstick_y) / (Math.sin(gyroDegrees) * leftstick_x)))); //360-270
        }
        else if(leftstick_x < 0 && leftstick_y < 0) { //quadrant up/left
            myangle = (float) (270 + Math.toDegrees(Math.atan((Math.cos(gyroDegrees) * leftstick_y) / (Math.sin(gyroDegrees) * leftstick_x)))); //270-180
        }
